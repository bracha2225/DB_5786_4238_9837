...
* **Database:** PostgreSQL 17.1 (Dockerized)
* **GUI / Admin:** pgAdmin 4
* **Backend API:** Python 3.12 (Flask)
* **Frontend Web Application:** HTML5, Vanilla CSS3, JavaScript (ES6)

---

## 📊 שלב ב' - שאילתות, אילוצים ואינדקסים
בשלב זה ביצענו תשאול מקיף של בסיס הנתונים ושיפרנו את רמת שלמות הנתונים והביצועים שלו:
* **שאילתות SELECT:** נכתבו 8 שאילתות מורכבות, מתוכן 4 נכתבו בשתי גרסאות שונות (גרסת Join מול תת-שאילתה/Exists/Lateral Join) עם ניתוח ביצועים מלא.
* **שאילתות עדכון ומחיקה:** נכתבו 3 שאילתות UPDATE ו-3 שאילתות DELETE העושות שימוש בלוגיקה קלינית ועקביות נתונים.
* **אילוצים חדשים:** נוספו אילוצי `CHECK` ו-`UNIQUE` משמעותיים לאבטחת נכונות הנתונים.
* **אינדקסים:** נוספו 3 אינדקסים על שדות חיפוש נפוצים (כגון מזהה מטופל, תאריכי אשפוז, ושמות אלרגיות), עם בדיקת זמני ריצה (EXPLAIN ANALYZE) המדגימה שיפור ביצועים של מאות אחוזים בשליפה.
* **Rollback & Commit:** הוכנו תרחישי בדיקה המראים את תהליך ביטול עסקאות (Rollback) ושמירתן (Commit) במסד הנתונים.

---

## 🔄 שלב ג' - אינטגרציה פיזית ומבטים
שילבנו את המערכת שלנו (Patients) עם מערכת צוות רפואי (Staff) של קבוצה מקבילה:
* **הנדוס לאחור (Reverse Engineering):** יצרנו תרשים ERD ו-DSD של האגף החדש מתוך קובץ הגיבוי שנתקבל.
* **אינטגרציה לוגית:** חיברנו את טבלת המטופלים לטבלת הסגל באמצעות קשר $1:N$ (כל מטופל משויך לאיש סגל רפואי אחראי).
* **אינטגרציה פיזית (`Integrate.sql`):** יצרנו סכמה נפרדת בשם `staffschema` לעבודה מבודדת והוספנו מפתח זר (`staffid`) מקשר.
* **מבטים (Views):**
  1. `view_patient_medical_assignments` - מבט אגף מטופלים המציג מטופל ופרטי הסגל הרפואי המשויך אליו.
  2. `view_staff_workload` - מבט אגף סגל המחשב את עומס המטופלים המשויך לכל עובד צוות.

---

## 💻 שלב ד' - תכנות בסיס הנתונים (PL/pgSQL)
הטמענו לוגיקה עסקית ומנגנוני בקרה אוטומטיים ישירות בתוך שרת ה-PostgreSQL:
* **פונקציה 1 (`evaluate_patient_risk`):** מחשבת את רמת הסיכון הרפואי של מטופל על סמך אבחנות ואלרגיות.
* **פונקציה 2 (`get_active_admissions`):** מחזירה `refcursor` דינמי של אשפוזים פעילים לפי מילת מפתח בסיבה.
* **פרוצדורה 1 (`bulk_discharge_patients`):** משחררת המונית מטופלים המאושפזים בשל סיבה ספציפית (למשל Flu).
* **פרוצדורה 2 (`archive_expired_insurance`):** מעבירה פוליסות ביטוח פגות תוקף לטבלת ארכיון ייעודית ומעודדת ניקוי נתונים.
* **טריגרים:**
  1. `trigger_admission_date_check` (BEFORE UPDATE) - מונע תאריכים עתידיים ומחשב שעות אשפוז ללוג.
  2. `trigger_prevent_duplicate_active_admission` (BEFORE INSERT) - מונע אשפוז כפול למטופל פעיל.

---

## 🖥️ שלב ה' - ממשק גרפי מודרני (GUI)
בשלב זה נבנתה אפליקציית ניהול מודרנית עם דגש על אסתטיקה ברמה הגבוהה ביותר:
* **עיצוב ואסתטיקה:** ממשק Glassmorphism יוקרתי, שימוש בגראדיאנטים וטשטוש שקפויות מבוסס CSS, תמיכה מלאה ב-RTL (עברית), התאמה לכל גודל מסך (רספונסיביות).
* **CRUD מלא לכל הטבלאות:**
  * הסתרת מפתחות זרים מספריים והחלפתם בשמות של נתונים (למשל: בחירת שם המטופל או שם הרופא מתוך Dropdown ייעודי).
  * מנגנון עדכון אוטומטי - לחיצה על ערוך שולפת את מפתח הרשומה ומציגה את שאר השדות למשתמש לצורך שינוי מהיר.
* **הרצת שאילתות ותוכניות:** מסך ייעודי המאפשר להריץ בלחיצת כפתור את שאילתות שלב ב' ואת הפרוצדורות/פונקציות של שלב ד'.

### 📸 צילומי מסך מהמערכת

#### 1. לוח הבקרה הראשי (Dashboard)
צפייה בנתונים סטטיסטיים בזמן אמת ממסד הנתונים:
![Dashboard Screenshot](DBProject/שלב%20ה/screenshots/dashboard.png)

#### 2. מסך ניהול מטופלים וטופס עריכה (CRUD & Update Form)
הצגת רשימת מטופלים (צירוף שמות רופאים) וטופס עדכון המציג את כל השדות אוטומטית לפי מפתח:
![Patients CRUD Screenshot](DBProject/שלב%20ה/screenshots/patients_crud.png)

#### 3. מסך הרצת שאילתות, פונקציות ופרוצדורות PL/pgSQL
חלון הרצה אינטראקטיבי עם פלט מודיעיני מותאם אישית:
![Queries Execution Screenshot](DBProject/שלב%20ה/screenshots/queries_run.png)

---

## 🚀 הוראות הרצה ושימוש בממשק
...
2. **הפעלת שרת ה-Web:**
   ...
   ```
3. **גישה למערכת:**
   פתחי את הדפדפן בכתובת: [http://localhost:5001](http://localhost:5001)
