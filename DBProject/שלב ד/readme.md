...
## 🛑 שלב 8: הוכחת אכיפת חוקים עסקיים וטיפול בשגיאות (Triggers Validation)

כדי להבטיח שהטריגרים שבנינו מגנים בצורה אבסולוטית על שלמות המידע, בוצעו ניסיונות מכוונים להזנת נתונים שגויים או כפולים. צילומי המסך הבאים מציגים כיצד המערכת חסמה את הפעולות והקפיצה את השגיאות המותאמות אישית שהגדרנו.

### 8.1 בדיקת טריגר חסימת כפילות אשפוזים (`trigger_prevent_duplicate_active_admission`)
בבדיקה זו ניסינו לבצע פקודת `INSERT` כדי לאשפז שוב את מטופל מספר 1 עם סיבה חדשה ('Broken arm'), בזמן שקיים עבורו אשפוז לב פעיל שטרם נסגר במערכת.
<br>• כפי שניתן לראות בצילום המסך, הטריגר התעורר מיד, חסם את הפעולה והקפיץ את השגיאה הברורה הבאה בחלונית ה-Messages:
`ERROR: Patient ID 1 is already currently admitted.`
עובדה זו מוכיחה שהאילוץ העסקי נאכף בהצלחה ומערכת האשפוזים מוגנת מפני כפילויות.

</p>

<img width="1112" height="732" alt="Capture d&#39;écran 2026-06-14 133220" src="https://github.com/user-attachments/assets/e1834ecd-134f-444c-9953-cd8671cb376f" />

<p align="right">

### 8.2 בדיקת טריגר מניעת תאריכים עתידיים (`trigger_admission_date_check`)
בבדיקה זו ניסינו לבצע פקודת `UPDATE` ולשנות באופן רטרואקטיבי את תאריך האשפוז של רשומה מספר 1 לתאריך פיקטיבי בעתיד הרחוק ('2035-12-25').
<br>• כפי שניתן לראות בצילום המסך, פונקציית הטריגר זיהתה את הניסיון להזנת תאריך לא הגיוני, ביטלה את הטרנזקציה והקפיצה את הודעת השגיאה המובנית שכתבנו:
`ERROR: Admission date cannot be set in the future.`
בכך הוכחה הגנה מלאה על אמינות תאריכי האשפוז של בית החולים.

</p>
<img width="1092" height="800" alt="Capture d&#39;écran 2026-06-14 133201" src="https://github.com/user-attachments/assets/582474ab-93d9-4347-a4de-6f61bd87a820" />
